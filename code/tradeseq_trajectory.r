...
CD4_sce1 <- slingshot(
    ...
    start.clus = "6", # Start trajectory from cluster 6
...
saveRDS(
    sce_gam,
    "/diazlab/data3/.abhinav/.immune/cancer_combined/project/cell_phenotyping/downstream2/CD8/CCA_integrated/trajectory/sce_clus6_fit_GAM.RDS"
)

# Generate a PDF of gene counts along the trajectory

# Extract necessary data for plotting
curves <- CD4_sds@curves # Ensure the curves object is defined properly
clustering <- colData(sce)$seurat_clusters # Use cluster labels from metadata
filt_counts <- counts[rownames(sce), ] # Filtered counts based on features in sce

# Plot the gene counts along the trajectory
pdf(paste0("/diazlab/data3/.abhinav/.immune/cancer_combined/project/cell_phenotyping/downstream2/CD8/CCA_integrated/trajectory/CD8_clus6_genes_count.pdf"))
